...
import { editEntry } from '../redux/accountsSlice'
...
// Custom imports
import { handleBuildLedgerItem, dateFormat, SlideTransition, categories } from "../utils/coreFunctions";
...
function EditItemModal(props) {
  ...
  useEffect(() => {
    if (props.editItemOpen !== false) {
      if (props.editItem.amount < 0) {
        setEntryChoice(false)
      ...
      setSelectedAmount(Math.abs(props.editItem.amount))
      setSelectedPayee(props.editItem.payee)
      handleDateChange(dateFormat(props.editItem.date))
      setCategorySelect(props.editItem.category)
    }
  }, [props])
  ...
  function handleEditItem(date, check, choice, category, payee, amount) {
    let entryItem = handleBuildLedgerItem(date, choice, category, payee, amount, check, props.selectedAccount)
    let index = accounts[props.selectedAccount].entries.findIndex(i => i.id === props.editItem.id)
    let data = {
      "itemIndex": index,
      "entry": entryItem
    }
  ...
  return (
    <form>
      <Dialog
        open={props.editItemOpen}
        onClose={props.handleEditItemClose}
          ...
          <DialogActions sx={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <Grid
              ...
              <Button
                ...
                color={buttonColor}
                sx={{ margin: '3px 5px' }}
                onClick={() => handleEditItem(selectedDate, checkNum, entryChoice, categorySelect, selectedPayee, selectedAmount)}>
                Update
              </Button>
              <Button
                variant="text"
                color={buttonColor}
                sx={{ margin: '3px 5px' }}
                onClick={props.handleEditItemClose}>
                Cancel
              </Button>
            </Grid>
          </DialogActions>
